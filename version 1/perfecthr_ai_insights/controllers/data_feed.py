...
When a data-fed model runs (e.g. performance_management), the runtime worker
calls back to PerfectHR for the employee's data — observed in the logs as:
  GET /api/performance-management/dataset-summary?dataset_id=<uuid>&employee_id=<id>
We answer with the employee's REAL performance data so the analysis is grounded
instead of falling back to "Analysis unavailable".
